...
const Nav = () => {
    ...
    const handleLogout = () => {
        setIsLoggedIn(false);
        // Future logout logic here (e.g., clear tokens)
    };
                ...
                <div className="flex justify-between items-center h-20">
                  ...
                  `}
                    ...
                    <div className="hidden md:flex items-center space-x-6 text-[#2a3731]">
                        ...
                        {/* User Account */}
                        <div className="relative group/user py-2 flex items-center">
                            <Link
                                to={isLoggedIn ? "/account/settings" : "/login"}
                                aria-label="User Account"
                                className="block transition-transform group-hover/user:scale-110 focus:outline-none"
                            >
                            ...
                            {/* User Dropdown Hover Panel */}
                            <div className="absolute right-0 top-full pt-4 w-56 opacity-0 invisible group-hover/user:opacity-100 group-hover/user:visible transition-all duration-300 transform origin-top-right z-50">
                                <div className="bg-white shadow-xl rounded-xl border border-[#e2e8e4] overflow-hidden p-2">

                                    {isLoggedIn ? (
                                        // Authenticated user view
                                        <div className="flex flex-col">
                                            <div className="px-3 py-2 text-xs tracking-wider uppercase font-medium text-[#8c9490] border-b border-[#e2e8e4] mb-1">
                                                My Account
                                            </div>

                                            <Link
                                                to="/account/settings"
                                                className="flex items-center px-3 py-2 text-sm text-[#2a3731] hover:bg-[#f2f4f3] rounded-md transition duration-200"
                                            >
                                                Settings
                                            </Link>
                                            ...
                                            <div className="border-t border-[#e2e8e4] mt-1 pt-1">
                                                <button
                                                    onClick={handleLogout}
                                                    className="w-full text-left flex items-center px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-md transition duration-200"
                                                >
                                                    Logout
                                                </button>
                                            </div>
                                        ...
                                        <div className="flex flex-col p-2 space-y-2">
                                            <div className="text-xs text-center text-[#8c9490] pb-1">
                                                Welcome to Velora
                                            </div>
                                            <Link
                                                to="/login"
                                                className="w-full block bg-[#3e564c] text-white py-2 rounded hover:bg-[#2a3731] transition duration-300 text-xs font-medium tracking-wide uppercase text-center"
                                            >
                                                Login
                                            </Link>
                                            <Link
                                                to="/register"
                                                className="w-full block border border-[#3e564c] text-[#3e564c] py-2 rounded hover:bg-[#f2f4f3] transition duration-300 text-xs font-medium tracking-wide uppercase text-center"
                                            >
                                                Register
                                            </Link>
                                        </div>
                                                ...
                                                {cart?.slice(0, 3).map((item: any, index: number) => (
                                                    <div key={item.id || index} className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <div className="text-xs text-[#8c9490] font-medium">${(item.price || 0).toFixed(2)}</div>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
